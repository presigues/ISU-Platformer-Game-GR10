...
public class EnemyManager {
	...
	private void drawDoggos(Graphics g, int xLvlOffset) {
		for(Doggo d : dog) {
		g.drawImage(doggoArr[d.getEnemyState()][d.getAniIndex()],
				(int) d.getHitbox().x - xLvlOffset - DOGGO_DRAWOFFSET_X + d.flipX() , 
				(int) d.getHitbox().y - DOGGO_DRAWOFFSET_Y, 
				ENEMY_WIDTH * d.flipW(), ENEMY_HEIGHT, null);
		d.drawHitbox(g, xLvlOffset);
		d.drawAttackBox(g, xLvlOffset);
		}
}
	private void loadEnemyImgs() {
		doggoArr = new BufferedImage[4][4];
